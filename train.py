...
# sgd를 사용할때 필요, --use_adam 을 한다면 --lr_step을 0으로 함 (학습시)
# 즉 adam을 쓸꺼면 별도의 learning rate 스케줄링을 할 필요가 없다.
    ...
    if config.lr_step > 0:
        lr_scheduler = optim.lr_scheduler.MultiStepLR(
            ...
            milestones=[i for i in range(
                ...
                config.lr_step # defalut 1 (매 epoch 마다)
            )],
            gamma=config.lr_gamma, # gamma라고 하는 애로 곱해서 점점 작아짐 (defalut 0.5)
