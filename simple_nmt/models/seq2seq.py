...
class Attention(nn.Module):
    ...
    def forward(self, h_src, h_t_tgt, mask=None):
        ...
        # (batch_size, 1, hidden_size) * (batch_size, hidden_size, length)
        ...
        weight = self.softmax(weight) # mini batch내 각 sample별 디코더의 현재 타임스탭에 대해서 인코더의 전체 타입스탭에 대해서 weight

        context_vector = torch.bmm(weight, h_src)
        # (batch_size, 1, length) * (batch_size, length, hidden_size)
        # |context_vector| = (batch_size, 1, hidden_size)
        # 미니배치내 각 sample별 디코더의 현재타입스탬에 대해서 어탠션 결과로 얻어온 context vector 
...
class Encoder(nn.Module):
    ...
    def forward(self, emb):
        ...
        y, h = self.rnn(x)
        # |y| = (batch_size, length, hidden_size) # 전체 timpestep의 마지막 layer의 hidden state
        # |h[0]| = (num_layers * 2, batch_size, hidden_size / 2) # 마지막 timestep의 hiddien state
...
class Decoder(nn.Module):
    ...
    def forward(self, emb_t, h_t_1_tilde, h_t_1):
        ...
        # h_t_1에는 hidden state과 cell state 둘 다 들어가 있음 --> 즉, RNN에 넣어줄때는 그 둘을 다 넣어 주어야 함
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        if isinstance(src, tuple):
            ...
            # |mask| = (batch_size, length) # boolean tensor
        else:
            x = src
    ...
    def search(self, src, is_greedy=True, max_length=255):
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            is_decoding = is_decoding * torch.ne(y, data_loader.EOS) # tesor 같으면 False, 다르면 True ==> EOS가 안나왔으면 True 나왔으면 False
