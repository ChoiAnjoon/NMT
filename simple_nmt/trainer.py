    ...
    def train(engine, mini_batch):
        ...
        if engine.state.iteration % engine.config.iteration_per_update == 0 and \
            ...
            if engine.config.gpu_id >= 0 and not engine.config.off_autocast:
                ...
                engine.optimizer.step() # 이 뒤에 learning rate 스케줄러 삭제 넣어줄 수 있음 

        loss = float(loss / word_count) # 단어당 loss (단어당 cross entropy값)
        ppl = np.exp(loss) # cross entropy에 exp 취하면 perplexity 값을 구할 수 있음 
