...
def read_text(batch_size=128):
    ...
    for line in sys.stdin:
        if line.strip() != '': # line이 존재할때 
            lines += [line.strip().split(' ')] # ' ' <-- 띄어쓰기로 split 
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for lines in read_text(batch_size=config.batch_size): # batch size의 갯수만큼 lines가 튀어나옴
            ...
            # |x| = (batch_size, length) # one hot vector로 나옴
